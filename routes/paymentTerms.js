...
module.exports = (models, requireLogin) => {
  ...
  router.post("/", requireLogin, async (req, res) => {
    try {
      ...
      const existing = await PaymentTerm.findOne({ where: { payment_term_name } });
      if (existing) {
        return res.status(409).json({ error: "Payment Term already exists" });
      }

      const newTerm = await PaymentTerm.create({ payment_term_name, description });
  ...
  router.get("/:id", requireLogin, async (req, res) => {
    try {
      const term = await PaymentTerm.findByPk(req.params.id);
  ...
  router.put("/:id", requireLogin, async (req, res) => {
    try {
      ...
      await term.update({ payment_term_name, description });
